...
Nhiệm vụ của mình phải tìm ba vector trong $\mathrm{GF}(p)$​ là $\varphi_A$​, $\varphi_B$ và $\varphi_C$​ sao cho tích có hướng của $\varphi^n_A$​ và $\varphi^m_B$​ bằng đúng $\varphi_C$​. Ở đây $\varphi^n_A$​ nghĩa là mỗi phần tử của $\varphi_A$​ được mũ $n \pmod p$​, $\varphi_A = (x_A, y_A, z_A)$ thì 

$$\varphi_A^n = (x^n_A \bmod p, y^n_A \bmod p, z^n_A \bmod p).$$
