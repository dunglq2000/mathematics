...
File `log.txt` khá dài nên mình lưu trong folder chứa project writeup này `TJCTF/assume`. Ở bài này mình chỉ cần kiểm tra xem if hay else được thực thi bằng việc kiểm tra message thứ 5 của mỗi đoạn có bằng `(g^a)^b` không.
...
### 6. lightweight-crypto-guard-system
...
Cứ tương tự vậy mình có $x_{i+n} = (A x_n + B) \bmod m$,

với $A = a^n \bmod m$ và $B = \dfrac{a^n - 1}{a - 1} \cdot b \bmod m$.
...
Với hint là file `keysmashes.txt` (mình cũng lưu trong repo này ở `TJCTF/hulksmash`) thì các dòng trong file sẽ giúp mình tìm key.
...
Writeup đến đây là kết thúc. Cám ơn các bạn đã đọc.